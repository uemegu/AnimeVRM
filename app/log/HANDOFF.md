...
- **UI & モーダル（白基調・ライトテーマ統一）**:
  ...
  - **夜の自室コミュニケーション（TV電話・メール）**:
    - 着信バイブ音（`phone_vibe.mp3`）ループ再生（応答・拒否時に即時停止）、ヒロインテーマカラー（黄・赤・青）の明るいアイコン背景を持つ着信通知カード（電話バッジ等なし・メッセージ非表示）および新着メール通知カード（本文プレビュー表示）。
    - 画面下部からスライドインするスマートフォン枠内でのTV電話（室内明ライティングの私服VRM顔アップ・インカメラ構図、CRT走査線ディスプレイ効果、通話時間、画面下部の通常DialogueBox・ChoiceBoxによるセリフと選択肢進行）。
    - スマートフォン枠内でのLINE風メール（トーク履歴、返信選択肢、相手のリアクション返信演出、既読表示）。
