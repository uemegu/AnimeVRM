...
- **実装ステータス**:
  ...
  - `app/Task.md` の「6. UI・画面フロー・基本操作」進行（主要UI全面刷新・洗練完了）
    ...
    - `ActionSelectModal.tsx`:
      ...
      - 行動選択カード: 番号・長文説明を撤廃し、ロケーション名＋遭遇バッジ＋2行固定高さINFOヒントボックス（上下中央揃え）で固定高さ86pxに統一。通常時はスクロールなし、画面が小さい場合はスリムスクロールバーで下まで操作可能
      - 移動決定前の自作YES/NOダイアログ（`ConfirmModal`）連携
    - `NightRoomView.tsx`: 「NIGHT PHASE」リボン、絵文字完全撤廃、青字見出し、ヒロイン別カラー好感度プログレスバー＆メーター、就寝ゴールドボタン、斜めカットコマンドボタン群
