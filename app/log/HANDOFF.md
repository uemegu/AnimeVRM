...
- **UI & モーダル（白基調・ライトテーマ統一）**:
  - **ヘッダー**: LOG（ノート）, AUTO（再生）, SHARE（Xロゴ/ショートカットS・セリフ合成キャプチャ付き・モバイルはWeb Share API、PCはクリップボード画像コピー＋1.5秒後Web Intentオープン）, MUTE（スピーカーON/OFF）, 言語切替（地球儀）, INFO。全ボタン（固定サイズ）とステータスバッジのサイズ統一、アイコンと文字の上下中央完全整列。
