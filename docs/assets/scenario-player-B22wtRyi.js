...
`,document.body.appendChild(I);var L=document.getElementById(`start-overlay`),R=document.getElementById(`replay-overlay`),z=document.getElementById(`start-btn`),B=document.getElementById(`replay-btn`),V=document.getElementById(`hud-restart-btn`);document.getElementById(`other-scenarios-select`)?.addEventListener(`change`,e=>{let t=e.target.value;t&&t!==E&&(window.location.href=`${t}.html`)});var H=!1,U=new m({scene:j.scene,camera:j.camera,controls:j.controls,avatarManager:M,audioLipSync:A,sharedEffectTextManager:j.sharedEffectTextManager,windController:k,panoramaController:j.panoramaController,getConfig:()=>O,onApplyConfig:e=>{P(e)},onSwitchScenePreset:e=>{F.switchScene(e,!1)},onFinished:()=>{H||R.classList.add(`visible`)}});async function W(){if(!H){H=!0,R.classList.remove(`visible`),L.classList.add(`hidden`);try{if(!D){r(`❌ シナリオ情報が見つかりません`);return}if(A.audioContext?.state===`suspended`)try{await A.audioContext.resume()}catch(e){console.warn(`Failed to resume AudioContext`,e)}U.scenarioEngine.isPlaying&&U.scenarioEngine.stop(),U.scenarioPlayer.isPlaying&&U.scenarioPlayer.stop(),j.panoramaController.isActive&&j.panoramaController.deactivate();let e=D.getScenario(`ja`);D.playOptions?.withInterlude?await U.playWithInterlude(e,{title:D.playOptions.interludeTitle||e.title,subtitle:D.playOptions.interludeSubtitle||`SPECIAL PRESENTATION`}):await U.scenarioEngine.play(e)}finally{H=!1}}}z.addEventListener(`click`,e=>{e.stopPropagation(),W()}),B.addEventListener(`click`,e=>{e.stopPropagation(),console.log(`[ScenarioPlayer] replayBtn clicked`),W()}),V.addEventListener(`click`,e=>{e.stopPropagation(),W()}),M.loadAvatarModel(M.currentModelUrl),F.switchTimeOfDay(`day`,!1);var G=new t;G.connect(document);function K(e){j.stats.begin(),G.update(e);let t=Math.min(G.getDelta(),.1),n=G.getElapsed();U.dialogueCameraController?.isActive?U.dialogueCameraController.update(t):M.animationPlayer.isPlaying?M.animationPlayer.update(t):j.panoramaController.isActive&&j.panoramaController.update(t,n),U.update(t);let r=U.dialogueCameraController?.isActive?U.dialogueCameraController.getBackgroundTransform():null;j.updateBackgroundZoom(r),j.updateMidgroundTransform(O,r),j.updateNeargroundTransform(O,r),U.scrollingBackgroundManager?.isVisible&&(U.scrollingBackgroundManager.update(t,r),j.midgroundMesh.visible=!1,j.neargroundMesh.visible=!1);let i=U.scenarioEngine.currentScene,a=i?.lipSyncCharacterId===void 0?i?.speakerCharacterId:i.lipSyncCharacterId??`none`;M.update(t,n,O,A,a),j.windParticles.update(t,n,O.wind,k.currentWindVector),j.rainEffect.setCameraPosition(j.camera.position),j.rainEffect.update(n);let o=M.getVrmMeshes();if(j.render(t,n,O,o),i?.live2d!==void 0){let e=i.live2d,t=typeof e==`boolean`?e:e.enabled??!0;N.setSceneOverride(t)}else N.setSceneOverride(null);N.update(t),j.stats.end(),requestAnimationFrame(K)}K(),window.addEventListener(`resize`,()=>{j.onResize()}),window.addEventListener(`pointerdown`,()=>{A.audioContext?.state===`suspended`&&A.audioContext.resume().catch(()=>{})},{once:!0});