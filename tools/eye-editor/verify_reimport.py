...
before={o.name:{k.name:[v.co.copy() for v in k.data] for k in o.data.shape_keys.key_blocks} for o in bpy.data.objects if o.type=='MESH' and o.data.shape_keys}
print('INIT',e.initialize());s=e.SESSION;params=dict(s['params'])
for src,obj in [(s['source'],s['preview']),*zip(s['source_lines'],s['lines'])]:
 assert all(v.co==before[src.name][k.name][i] for k in obj.data.shape_keys.key_blocks for i,v in enumerate(k.data)),obj.name
...
for control,value in [('face_slim',.5),('eye_x',.3),('thickness',2),('corner_angle',.5)]:
 ...
 if control in ('thickness','corner_angle','eye_x'):
  assert any(v.co!=before[s['source_lines'][0].name]['Basis'][i] for i,v in enumerate(s['lines'][0].data.shape_keys.key_blocks[0].data))
e.update(params)
for src,obj in [(s['source'],s['preview']),*zip(s['source_lines'],s['lines'])]:
 assert all(v.co==before[src.name][k.name][i] for k in obj.data.shape_keys.key_blocks for i,v in enumerate(k.data)),obj.name
 assert all(v.co==before[src.name][k.name][i] for k in src.data.shape_keys.key_blocks for i,v in enumerate(k.data)),src.name
