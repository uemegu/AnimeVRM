...
const browser=await chromium.launch({headless:true,args:['--autoplay-policy=no-user-gesture-required','--use-fake-ui-for-media-stream','--use-fake-device-for-media-stream',`--use-file-for-fake-audio-capture=${process.cwd()}/anime.wav`]});
...
await page.locator('#audioFile').setInputFiles('anime.wav');await page.locator('#audioPlay').click();
