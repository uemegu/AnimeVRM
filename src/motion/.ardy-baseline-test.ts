...
$('add-joint').onclick = () => void action(async () => {
  ...
  const layer = newLayer('@joint', engine, recipe.duration);
  recipe.layers.push(layer); selected = layer.id; solo = null; time = 0; playing = false; syncPlay(); render();
  status('関節と角度を選んでください。カードを追加すると複数の関節を調整できます。');
});
...
function render(save = true) {
  ...
  $('layers').innerHTML = recipe.layers.length ? recipe.layers.map((l, index) => `<article class="layer ${l.id === selected ? 'selected' : ''} ${l.enabled ? '' : 'muted'}" data-id="${l.id}"><div class="layer-top"><input data-field="enabled" type="checkbox" ${l.enabled ? 'checked' : ''} aria-label="${nameOf(l.source)}を有効にする"><button class="layer-title" data-action="select" aria-expanded="${!closedDetails.has(l.id)}"><span class="number">${String(index + 1).padStart(2, '0')}</span>${nameOf(l.source)}</button><select data-field="mask" aria-label="適用部位">${masks.map(m => `<option ${l.mask === m ? 'selected' : ''}>${m}</option>`).join('')}</select><label class="weight">強さ <input data-field="weight" type="range" min="0" max="1" step=".01" value="${l.weight}" aria-label="強さ"><output>${Math.round(l.weight * 100)}%</output></label><button data-action="up" aria-label="上へ移動" ${index === 0 ? 'disabled' : ''}>↑</button><button data-action="down" aria-label="下へ移動" ${index === recipe.layers.length - 1 ? 'disabled' : ''}>↓</button><button data-action="copy" aria-label="複製">⧉</button><button data-action="delete" aria-label="削除">×</button></div>${placement(l.id)}${!closedDetails.has(l.id) ? `<div class="details">${l.source === '@joint' ? jointControls(l) : ''}${numeric('区間の長さ', 'duration', l.duration, .1, 60, .1, '秒')}${numeric('速度', 'speed', l.speed, .1, 3, .1, '×')}${numeric('元の開始', 'from', l.from, 0, l.to - 1, 1, 'f')}${numeric('元の終了', 'to', l.to, l.from + 1, engine.frames(l.source), 1, 'f')}${numeric('フェード', 'fade', l.fade, 0, 5, .1, '秒')}<label><input data-field="loop" type="checkbox" ${l.loop ? 'checked' : ''}>素材を繰り返す</label>${(l.source.startsWith('@') || l.source.startsWith('saved:')) && l.mask === '全身' ? `<label><input data-field="balance" type="checkbox" ${l.balance !== false ? 'checked' : ''}>全身のバランス補助</label>` : ''}${l.source.startsWith('@') && l.source !== '@joint' ? `<label>基本動作の再生<select data-field="poseMode" aria-label="基本動作の再生"><option value="motion" ${l.poseMode !== 'hold' ? 'selected' : ''}>動作として再生</option><option value="hold" ${l.poseMode === 'hold' ? 'selected' : ''}>最後の姿勢を使う</option></select></label>` : ''}${l.source === '@raise' || l.source === '@hair' || basics[l.source]?.reach ? `<label class="contact-gap">腕に連動する肩<input data-field="shoulderFollow" aria-label="腕に連動する肩" type="range" min="0" max="1" step=".05" value="${l.shoulderFollow ?? 1}"><output>${Math.round((l.shoulderFollow ?? 1) * 100)}%</output></label>` : ''}${l.source === '@hair' || basics[l.source]?.reach?.contact ? `<label class="contact-gap">体から離す距離（cm）<input data-field="contactGap" aria-label="体から離す距離" type="range" min="-10" max="20" step=".5" value="${l.contactGap ?? 0}"><output>${(l.contactGap ?? 0).toFixed(1)} cm</output></label>` : ''}<small>元のフレームは30fps換算。素材の繰り返しと、タイムライン上の区間の繰り返しは別々に設定できます。</small>${trimControls(l.id)}</div>` : ''}</article>`).join('') : '<div class="empty">左から動作を追加してみましょう。<br><small>プリセットなら、ワンクリックで始められます。</small></div>';
  if (save) persist();
}
function jointControls(layer: Recipe['layers'][number]) {
  return `<div class="joint-controls"><b>関節の角度</b><label>関節<select data-field="joint" aria-label="関節">${Object.entries(jointNames).filter(([bone]) => engine.rest?.has(bone)).map(([bone, name]) => `<option value="${bone}" ${bone === (layer.joint ?? 'RightForeArm') ? 'selected' : ''}>${name}</option>`).join('')}</select></label><label>回転の基準<select data-field="jointSpace" aria-label="回転の基準"><option value="local" ${layer.jointSpace !== 'world' ? 'selected' : ''}>関節自身の軸</option><option value="world" ${layer.jointSpace === 'world' ? 'selected' : ''}>空間の軸</option></select></label>${(['X', 'Y', 'Z'] as const).map(axis => {
    const key = `joint${axis}` as 'jointX' | 'jointY' | 'jointZ';
    return `<label class="joint-angle">${axis}軸<span><input data-field="${key}" aria-label="${axis}軸スライダー" type="range" min="-180" max="180" step="1" value="${layer[key] ?? 0}"><input data-field="${key}" aria-label="${axis}軸の角度" type="number" min="-180" max="180" step="1" value="${layer[key] ?? 0}"> °</span></label>`;
  }).join('')}${numeric('補間時間', 'jointEase', layer.jointEase ?? .7, 0, 10, .1, '秒')}<button data-action="joint-reset">角度をリセット</button><small>開始と終了を補間時間で滑らかにつなぎます。全体をループしない場合、本編の最後まで続くカードは姿勢を保持します（0秒で即適用、短い区間は半分の長さに調整）。元の動きに回転を追加します。空間の軸は X＝左右、Y＝上下、Z＝前後。関節自身の軸は姿勢や左右で向きが変わります。複数関節はカードを追加。適用部位に含まれる関節だけに反映されます。</small></div>`;
}
...
$('layers').addEventListener('input', event => {
  ...
  if (key === 'joint' || key === 'jointSpace') {
    if (key === 'joint') layer.joint = input.value; else layer.jointSpace = input.value as 'local' | 'world';
    solo = null; persist(); return;
  }
  if (key === 'jointX' || key === 'jointY' || key === 'jointZ') {
    if (!Number.isFinite(input.valueAsNumber)) return;
    layer[key] = T.MathUtils.clamp(input.valueAsNumber, -180, 180);
    article!.querySelectorAll<HTMLInputElement>(`[data-field="${key}"]`).forEach(peer => { if (peer !== input) peer.value = String(layer[key]); });
    solo = null; persist(); return;
  }
  ...
  if (key === 'repeat-enabled') { layer.repeatEvery = input.checked ? Math.min(60, layer.duration * 2) : 0; }
  ...
  else if (['weight', 'duration', 'speed', 'from', 'to', 'fade', 'repeatEvery', 'contactGap', 'shoulderFollow', 'jointEase'].includes(key) && input.value !== '' && Number.isFinite(input.valueAsNumber)) {
    ...
    if (key === 'weight' || key === 'shoulderFollow') input.nextElementSibling!.textContent = `${Math.round(v * 100)}%`;
  }
  persist();
});
...
$('layers').onclick = event => {
  ...
  if (act === 'joint-reset') { layer.jointX = layer.jointY = layer.jointZ = 0; solo = null; }
