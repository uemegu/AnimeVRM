...
export class SkyBackground {
  ...
  setTimeOfDay(time?: string): void {
    const colors = time === 'divine'
      ? ['#0f172a', '#3b4252', '#fed7aa', '#1e293b']
      : time === 'evening'
    ...
    this.material.uniforms.uSkyGlow.value = time === 'divine' ? 0.35
      : (time === 'dark_indoor' || time === 'night') ? 0.12
  ...
  setTransform(transform?: { zoomScale: number; panOffsetX: number; panOffsetY: number } | null): void {
    const repeat = 1 / Math.max(1, transform?.zoomScale ?? 1);
    this.material.uniforms.uRepeat.value.set(repeat, repeat);
    this.material.uniforms.uOffset.value.set(
      (1 - repeat) * 0.5 - (transform?.panOffsetX ?? 0),
      (1 - repeat) * 0.5 - (transform?.panOffsetY ?? 0),
    );
  }
}
